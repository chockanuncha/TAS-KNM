...
#Region "Windows"
    ...
    Private Sub SelectVLoadingNote()
        ...
        Try
            ...
            q &= " ,max(T1.ADDNOTEDATE) as ADDNOTEDATE,max(T2.LC_STARTTIME) as LC_STARTTIME,max(T2.LC_ENDTIME) as LC_ENDTIME,min(RAW_WEIGHT_IN) as RAW_WEIGHT_IN "
    ...
    Private Sub Adddata_CLICK(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Adddata.Click
        ...
        Try
            ...
            Loadid.Text = dt_tmp(0)("Load_id")
            Reference.Text = dt_tmp(0)("Reference")
    ...
    '        loadid.text = tmp(0)("Load_id")
    '        Reference.text = tmp(0)("Reference")
    '        Cbn11.Focus()
    '        'RadPageView1.SelectedPage = RadPageViewPage2
...
#Region "Select_Bay"
    ...
    Private Sub PresetVal_TextChanged(sender As Object, e As EventArgs) Handles PresetVal.TextChanged
        Try
            If Int(PresetVal.Text) > Int(Capacity.Text) Then
                PresetVal.Text = Capacity.Text
            End If
        Catch ex As Exception
        End Try
    End Sub

    Private Sub PresetTotal_KeyPress(sender As Object, e As KeyPressEventArgs)
        If IsNumeric(e.KeyChar) = False AndAlso e.KeyChar <> Chr(8) Then
            e.KeyChar = Chr(0)
        End If
    End Sub

    Private Sub PresetVal_Leave(sender As Object, e As EventArgs) Handles PresetVal.Leave
        Dim tov As Integer
    ...
    Private Sub PresetVal_KeyPress(sender As Object, e As KeyPressEventArgs) Handles PresetVal.KeyPress
        If IsNumeric(e.KeyChar) = False AndAlso e.KeyChar <> Chr(8) Then
            e.KeyChar = Chr(0)
        End If
    End Sub
    ...
    Private Sub Advisenote_Topup_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
    ...
    Private Sub Product_Leave(sender As Object, e As EventArgs) Handles Product.Leave
        Try
            Dim ProductID As String = ""
            Dim sql As String
            Dim ProductCount As Integer = 0
            ...
            sql = ""
            'sql = "select bay_number,bay_status  from t_bay where bay_status=1 and  bay_meter1 in(select batch_number from t_batchmeter where batch_pro='" & Product_ID & "' and batch_Status='10') or "
            sql = "select bay_number,bay_status  from t_bay where bay_status=1 and [BAY_QUEUE] <> 0 and  bay_meter1 in(select batch_number from t_batchmeter where batch_pro='" & Product_ID & "' and batch_Status='10') or "
            sql &= " bay_meter2 in(select batch_number from t_batchmeter where batch_pro='" & Product_ID & "' and batch_Status='10') or "
            sql &= "bay_meter3 in(select batch_number from t_batchmeter where batch_pro='" & Product_ID & "' and batch_Status='10') or "
            sql &= "bay_meter4 in(select batch_number from t_batchmeter where batch_pro='" & Product_ID & "' and batch_Status='10') or "
            sql &= "bay_meter5 in(select batch_number from t_batchmeter where batch_pro='" & Product_ID & "' and batch_Status='10') or "
    ...
    Private Sub Bay_Click(sender As Object, e As EventArgs) Handles Bay.Click
        Try
            'Bay.MultiColumnComboBoxElement.ShowPopup()
        Catch ex As Exception
        End Try
...
#Region "Save,Update,Delete"
    ...
    Private Sub Bsave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Bsave.Click
        ...
        If PresetVal.Text = "" Or PresetVal.Text = "0" Then
            MessageBox.Show("Please specify Preset", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information)
            PresetVal.Focus()
            Exit Sub
        End If
        ...
        Try
            ...
            If Cbn7.Text = "" Then
                Try
                    ...
                    Dim dt12 As DataTable = cls.Query(sql)
                    q &= "'" & dt12(0)("Q_NO").ToString & "',"
                Catch ex As Exception
                    q &= "'" & ("0") & "',"
                End Try
            Else
            ...
            ''''''''' End Insert T_LoadingnoteCompartment
            ...
            Try
                ...
                Dim dtx As DataTable = cls.Query(sql)

                DriverCard = dtx(0)("Card_Number")
                q &= "" & DriverCard & "',"
            Catch ex As Exception
                q &= "'" & ("0") & "',"
            End Try
            ...
            Try
                ...
                Dim dtx2 As DataTable = cls.Query(sql)
                If dtx2.Rows.Count > 0 Then TruckCard = dtx2(0)("Card_Number") Else TruckCard = "0"
        ...
        q = "Update t_Checkin Set Status=2 ,LOAD_ID='" & Loadid.Text & "',Call_target='" & Call_Tergets.ToString & "' where ID='" & Checkin_ID & "'"
        cls.Update(q)
        ...
        If AddDoc Then
            cls.InsertEvent(MAIN.U_NAME, "User : " & MAIN.U_NAME & "  Create loading advise note : " & Reference.Text, "T_LOADINGNOTE", "")
            EditDoc = False
            AddDoc = False
        ElseIf EditDoc Then
            cls.InsertEvent(MAIN.U_NAME, "User : " & MAIN.U_NAME & "  Edit loading advise note : " & Reference.Text, "T_LOADINGNOTE", "")
            EditDoc = False
            AddDoc = False
        End If
    ...
    Private Sub Editdata_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Editdata.Click
        ...
        Else
            ...
            ' Weight
            q &= "T_LOADINGNOTE.RAW_WEIGHT_IN as RAW_WEIGHT_IN ,"
            q &= "T_LOADINGNOTE.RAW_WEIGHT_Out as RAW_WEIGHT_Out ,"
            q &= "T_LOADINGNOTE.WEIGHTin_Time as WEIGHTin_Time ,"
            q &= "T_LOADINGNOTE.WEIGHTin_Time as WEIGHTout_Time "

            q &= "FROM "
            q &= "(Select * from T_LOADINGNOTE where Reference = '" & ref & "' and load_Status in(1,2,3,4,5) ) T_Loadingnote "
            ...
            Try
                ...
                q = "Select  T1.LC_ID As LC_ID,T1.LC_LOAD As LC_LOAD,T1.LC_PRO As LC_PRO,T1.LC_TANKETH As LC_TANKETH,T1.LC_TANK As LC_TANK,T1.LC_SEAL As LC_SEAL " +
                ",T5.PRODUCT_CODE AS PRODUCT_CODE,T1.LC_COMPARTMENT AS LC_COMPARTMENT,T1.LC_STATUS AS LC_STATUS,T1.LC_CAPACITY AS LC_CAPACITY,T1.LC_PRESET AS LC_PRESET " +
                 ",T1.LC_METER AS LC_METER,T1.LC_BAY AS LC_BAY,T1.LC_ISLAND AS LC_ISLAND,T1.LC_STARTTIME AS LC_STARTTIME,T1.LC_ENDTIME AS LC_ENDTIME" +
      ",T1.LC_TEMP AS LC_TEMP,T1.LC_DENSITY_30C AS LC_DENSITY_30C,T1.LC_VCF AS LC_VCF,T1.LC_METERFACTOR3 AS LC_METERFACTOR3,T1.LC_K_FACTOR AS LC_K_FACTOR " +
      ",T1.LC_BASE AS LC_BASE,T1.LC_BLEND AS LC_BLEND,T1.LC_BATCHNET_15C AS LC_BATCHNET_15C,T1.LC_CUMULNET_15C AS LC_CUMULNET_15C,T1.LC_TANKGB AS LC_TANKGB " +
      ",T1.LC_API AS LC_API,T1.LC_TEMP_MANUAL AS LC_TEMP_MANUAL,T1.LC_SHIPPER AS LC_SHIPPER,T4.SP_CODE AS SP_CODE,T1.LC_CUSTOMER AS LC_CUSTOMER " +
      ",T3.CUSTOMER_CODE AS CUSTOMER_CODE, T2.BATCH_NAME AS BATCH_NAME,T1.LC_ADDITIVE AS LC_ADDITIVE,T3.CUSTOMER_NAME AS CUSTOMER_NAME " +
        "FROM (select * from T_LOADINGNOTECOMPARTMENT where lc_load= " & Loadid.Text & ") T1 " +
         "Left OUTER JOIN T_BATCHMETER T2    On T1.LC_METER = T2.BATCH_NUMBER  " +
         "Left OUTER JOIN T_CUSTOMER T3      On T1.LC_CUSTOMER = T3.ID  " +
         "Left OUTER JOIN T_SHIPPER T4   On T1.LC_SHIPPER = T4.ID  " +
         "Left OUTER JOIN T_PRODUCT T5   On T1.LC_PRO = T5.ID "
                ...
                For i = 0 To dt1.Rows.Count - 1
                    If (dt1.Rows(i).Item("Product_code").ToString) <> "" Then
                        productlist = ""
                        productlist = (dt1.Rows(i).Item("Product_code").ToString)
                        index = DirectCast(Me.GroupBox14.Controls.Item("ProductList" + (i + 1).ToString), RadDropDownList).FindString(productlist)
